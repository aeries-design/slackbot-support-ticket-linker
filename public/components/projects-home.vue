    ...
    .flex-col(class="w-full bg-neutral-300 content_container")
      ...
      section.content(class="w-full mt-8")
        div(class="px-12 xl:px-24")
          section
              div(class="flex mt-8")
                img(v-if="allSlackUsers" :src="mySlackObject.profile.image_192").rounded-full.w-20.h-20.self-center
                div(class="block p-4")
                  h2(class="text-title md:text-header font-bold font-aeries") {{currentUser}}
                  button(@click="showAddDropdownBox = !showAddDropdownBox" id="add" class="bg-blue-700 rounded-full px-8 py-1 text-white") + Add 
                    .dropdown(v-if="showAddDropdownBox" class="absolute text-blue-600 text-left bg-white shadow-lg w-48")
            ...
            table(class="w-full table-fixed")
              thead
                tr.border-b-4.border-neutral-600
                  ...
                  th(class="text-subhead p-4 text-left w-4/12") Parent
              ...
              tbody
                tr(v-for="task in myIncompleteTasks" class="border-b border-neutral-500")
                  ...
                  td(class="p-4 text-left")
                    a(class="text-blue-700 font-semi-bold cursor-pointer" @click="completeTask(task)") Complete task


      section.content(class="w-full mt-8")
        div(class="px-12 xl:px-24")
          
          section.session(v-for="session in myObjectivesAcrossSessions" v-if="session.childObjectivesAssignedToMe.length || session.childObjectivesAssignedToMe.length")            
            .objectiveTables
              .objectiveTable.assignedToMe(class="mb-12")
                h1(class="text-title mb-4") Objectives assigned to me in
                  a(:href="'/projects/goals/'+session._id")
                    img(class="inline w-6 ml-2 mb-1 mr-1" src='/images/session.svg')
                    span(class="text-blue-800 font-bold font-aeries") {{session.title}}
                table(class="w-full table-fixed")
                  thead
                    tr(class="border-b-4 border-neutral-600")
                      th(class="text-subhead p- pl-04 text-left w-3/12") Objective & owner
                      th(class="text-subhead p-4 text-left w-1/12") Contains
                      th(class="text-subhead p-4 text-left w-4/12") Key results
                      //- th(class="font-aeries text-subhead p-4 text-left w-1/12") Progress
                      th(class="text-subhead p-4 text-left w-4/12") Description
                  tbody
                    tr(v-for="objective in session.childObjectivesAssignedToMe" class="border-b border-neutral-500")
              ...
              .objectiveTable.assignedToMyTeams(class="mb-12" v-if="session.childObjectivesAssignedToMyTeams.length")
                h1(class="text-title mb-4") Objectives assigned to my teams in 
                  a(:href="'/projects/goals/'+session._id")
                    img(class="inline w-6 ml-2 mb-1 mr-1" src='/images/session.svg')
                    span(class="text-blue-800 font-bold font-aeries") {{session.title}}
                ...
                table(class="w-full table-fixed")
                  thead
                    tr(class="border-b-4 border-neutral-600")
                      th(class="text-subhead p-4 pl-04 text-left w-3/12") Objective & owner
                      th(class="text-subhead p-4 text-left w-1/12") Contains
                      th(class="text-subhead p-4 text-left w-4/12") Key results
                      //- th(class="font-aeries text-subhead p-4 text-left w-1/12") Progress
                      th(class="font-aeries text-subhead p-4 text-left w-4/12") Description
                  tbody(v-if="session.childObjectivesAssignedToMyTeams.length")
                    tr(v-for="objective in session.childObjectivesAssignedToMyTeams" class="border-b border-neutral-500")
                      td(class="p-4 text-left pr-8")
                        .title(v-if="objective.title" class="font-semi-bold text-subhead text-blue-600 pb-4 cursor-pointer flex" @click="selectObjective(objective)") 
                          .icon(class="w-1/12 mr-3")
                            img(class="inline" src='/images/goal.svg')
                      ...
                      td(class="p-4 text-left")
                        span(v-if="objective.description") {{objective.description | truncate(230, '...')}}
                        span(v-else) -
                  p(v-else class="my-4 text-minimum-text text-neutral-1200") No objectives found belonging to any teams I'm a member of.
