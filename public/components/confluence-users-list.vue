...
.container.mx-auto.grid.grid-cols-1.row-gap-16.px-4.py-4(class='sm:px-6 sm:py-12 lg:px-8')
  #product-marketing
    div
      ...
      div
        h2.text-title.font-aeries.font-semi-bold Copy and paste to a Confluence page to tag a large number of users
        //- Split the tag list by " @" to get the number of selected users
        p(class="text-minimum-text mt-2" v-if="confluenceTagList.length") {{confluenceTagList.split(' @').length}} selected
        p(class="text-minimum-text mt-2" v-else) 0 selected
        textarea(v-model="confluenceTagList" class="w-full mb-6 p-2 bg-neutral-400")
      ...
      div(class='sm:flex sm:items-baseline sm:justify-between')
        h2.text-display.leading-8.font-semi-bold.tracking-tight.font-aeries.text-gray-900(class='sm:text-3xl sm:leading-9')
          | Confluence users - {{activeUsers.length}}
        ...
        div(class='grid grid-cols-1 row-gap-4 sm:grid-cols-2 sm:col-gap-2 sm:row-gap-2 md:grid-cols-4 lg:col-span-4')
          ...
          div(class='flex user-item p-2 cursor-pointer border-dashed border-2 border-transparent hover:border-neutral-500' v-for="(user, index) in activeUsers" @click="selectUser(user, index, $event)" v-bind:class="{ 'bg-neutral-500' : user.selected }")
            .flex-1.leading-snug.w-0
              h4(class='hover:text-primary whitespace-no-wrap text-subhead font-aeries text-secondary font-bold truncate') {{user.name}}
              p(class='text-body text-neutral-1600 text-minimum-text') {{user.username}} - {{user.email}}
...
computed : {
  numberOfSelectedUsers() {
    return this.confluenceTagList.split(' @').length;
  },
  confluenceTagList() {
    var confluenceTagList = [];
    for (var i = 0; i < this.users.length; i++) {
      if (this.users[i].selected) {
        confluenceTagList.push("@" + this.users[i].username);
      }
    }
    return confluenceTagList.join(" ");
  },
  activeUsers() {
...
async mounted () {
      ...
      axios
      .get('/rest/users?$limit=1000&platform=confluence')
