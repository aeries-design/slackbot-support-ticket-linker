...
main
  ...
  .flex.w-full(v-show="selectedPage == 'home'")
    .flex-col(class="w-full bg-neutral-300")
      .loader(v-if="showLoader")
        ...
        .loadingOverlay(class="bg-neutral-2000 opacity-75 absolute" style="height: 100vh; width: 100vw;")
      #home.all-content(class="")

      section.content(class="w-full mt-8")
        div(class="px-12 xl:px-24")
          section
              div
                h2(class="text-header md:text-display font-bold font-aeries") My tasks for WoolyMango
                //- button(@click="showAddDropdownBox = !showAddDropdownBox" id="add" class="bg-blue-700 rounded-full px-8 py-1 text-white") + Add 
                button(@click="showAddTask()" id="add" class="bg-blue-700 rounded-full px-8 py-1 text-white") + Add 
                .dropdown(v-show="showAddDropdownBox" class="absolute text-blue-600 text-left bg-white shadow-lg w-48")
                  div(class="hover:bg-neutral-400 p-3 font-bold" @click="showAddTask()") 
                    img(class="inline w-5 pr-1" src='/images/task.svg')
                    span Task
                  div(class="hover:bg-neutral-400 p-3 font-bold" @click="showAddObjective()") 
                    img(class="inline w-5 pr-1" src='/images/goal.svg')
                    span Objective
              hr(class="mb-4 mt-8 border-neutral-800")
              div.me(class="flex mt-8" v-if="mySlackObject")
                img(v-if="allSlackUsers" :src="mySlackObject.profile.image_192").rounded-full.w-6.self-center
                div(class="block p-2")
                  h2(class="font-bold font-aeries") My active task for WoolyMango
              .noTasksLabel(v-if="!myActiveDesignTasks.length")
                span.italic.text-minimum-text You currently have no active task set for WoolyMango team to work on. Add one above, or choose from an existing one below.
              .taskCards(v-if="myActiveDesignTasks.length" class="flex mt-3")
                .card(v-for="task in myActiveDesignTasks" @click="$refs.projectsModal.backToObjectiveRef = {}; selectTask(task);" class="w-1/2 lg:w-1/5 border-neutral-600 border rounded shadow hover:shadow-lg p-4 mr-4 cursor-pointer")
                  .iconTitle
                    img(class="w-6 mb-2" src='/images/task.svg')
                    span(class="text-minimum-text font-bold") {{task.title}}
                  .taskStatus(class="text-minimum-text text-white inline-block rounded pt-2 p-1 my-2 uppercase" :class="{'bg-neutral-1200' : task.status == 'To-do', 'bg-blue-600' : task.status == 'In progress', 'bg-green-600' : task.status == 'Done'}") 
                    | {{task.status}}
                  .taskDate(v-if="task.due_date" class="text-minimum-text text-neutral-800 mt-2") Due on or before 
                    span(class="font-bold") {{task.due_date}}
          
          section(class="mt-16")
            h2(class="text-subhead md:text-title font-bold font-aeries") My WoolyMango task backlog
            table(class="w-full table-fixed")
              thead
                tr.border-b-4.border-neutral-600
                  th(class="text-subhead p-4 pl-0 text-left w-6/12") To-do
                  //- th(class="font-aeries text-subhead p-4 text-left w-2/12") Source
                  th(class="text-subhead p-4 text-left w-2/12") Due
                  th(class="text-subhead p-4 text-left w-4/12") Parent objective
                  th(class="text-subhead p-4 text-left w-2/12") Action
              tbody
                tr(v-for="task in myIncompleteTasks" class="border-b border-neutral-500")
                  td(class="py-4 text-left pr-8") 
                    div.flex
                      ...
                      .title(class="text-blue-700 cursor-pointer"
                      @click="$refs.projectsModal.backToObjectiveRef = {}; selectTask(task);") 
                        | {{task.title}}
                  //- td(class="p-4 text-left") {{task.type}}
                  td(class="p-4 text-left") {{task.due_date}}
                  td(class="p-4 text-left title font-semi-bold text-blue-600")
                    a(v-if="task.linked_to_objective_id && fetchDataElementByID('objective', task.linked_to_objective_id)" class="flex cursor-pointer" @click="selectObjective(fetchDataElementByID('objective', task.linked_to_objective_id))") 
                      img(class="inline w-6 self-start mr-2" src='/images/goal.svg')
                      span {{fetchDataElementByID('objective', task.linked_to_objective_id).title | truncate(90, '...')}}
                  td(class="p-4 text-left")
                    a(class="text-blue-700 font-semi-bold cursor-pointer" @click="setNewActiveTask(task)") Set active task
...
setNewActiveTask(newTask) {
    //Get the list of active tasks

    //For each item in the list, delete activeDesignTask.
    //Check if i == the length of myActiveDesignTasks and if so, set
    //the selected task as the only active task.

if (confirm("Do you want to make this your active task, replacing the one that you have currently set (if any)?")) {

    var i = 0;
    var tasks = this.myActiveDesignTasks;
    ...
    function unsetActiveTask(task) {
      var taskPayload = Object.assign({}, task);
      delete taskPayload.editHistory;
      delete taskPayload.activeDesignTask;
      taskPayload.recordUpdateBy = globalScope.currentUser.id;

      axios
      .put('/rest/okrs/' + taskPayload._id, taskPayload)
      .then(function(response) {
        if ((i == (taskLength - 1)) || taskLength == 0) {
        //We're done, set the task as the active task now

        var taskPayload = Object.assign({}, newTask);
        delete taskPayload.editHistory;
        taskPayload.recordUpdateBy = globalScope.currentUser.id;
        taskPayload.activeDesignTask = "true";
        axios
        .put('/rest/okrs/' + taskPayload._id, taskPayload)
        .then(function(response) {
          globalScope.refreshOKRData();
          alert("Active task updated.")
        })

        } else {
          i++
          unsetActiveTask(tasks[i])
        }
    ...
    unsetActiveTask(tasks[i]);
        ...
        console.log("No existing active tasks")
        var taskPayload = Object.assign({}, newTask);
        delete taskPayload.editHistory;
        taskPayload.recordUpdateBy = globalScope.currentUser.id;
        taskPayload.activeDesignTask = "true";
                    ...
                    axios
                      ...
                      .then(function (response) {
                        ...
                        if (executeQueryParameters) {
                          ...
                          //Active design tasks
                          axios
                            .get('/rest/okrs?type=Aeriesworks%20Task&$limit=9999&deleted[$ne]=true&activeDesignTask=true&recordCreatedBy='+globalScope.currentUserID)
                            .then(function (response) {
                              globalScope.myActiveDesignTasks = response.data.data;
                            })
